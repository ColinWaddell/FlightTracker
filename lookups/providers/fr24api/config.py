...
PROVIDER = ProviderConfig(
    ...
    description=(
        ...
        "live-positions call bills roughly 8 credits per returned record with "
        "a 1-credit minimum even when nothing is returned - a ten-flight zone "
        "poll costs ~80 credits, and rapid re-calls trip the burst limit "
        "(HTTP 429). Responses carry x-fr24-credits-consumed / -remaining "
        "headers for monitoring. Best suited to occasional route/aircraft "
        "lookups; leave continuous polling to the free providers."
    ),
